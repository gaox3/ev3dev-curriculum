...
class Snatch3r(object):
    ...
    def turn_degrees(self, degrees_to_turn, turn_speed_sp):
        if degrees_to_turn > 0:
            self.left_motor.run_to_rel_pos(speed_sp=(-turn_speed_sp), position_sp=-5.2*degrees_to_turn,
                                           stop_action=ev3.Motor.STOP_ACTION_BRAKE)
            self.right_motor.run_to_rel_pos(speed_sp=turn_speed_sp, position_sp=5.2*degrees_to_turn,
                                            stop_action=ev3.Motor.STOP_ACTION_BRAKE)
        else:
            self.left_motor.run_to_rel_pos(speed_sp=turn_speed_sp, position_sp=5.2*degrees_to_turn,
                                           stop_action=ev3.Motor.STOP_ACTION_BRAKE)
